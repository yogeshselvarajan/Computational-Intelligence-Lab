...
def uniformCostSearch(graph, startnode, endnode):
  visited = set()
  queue = deque([(startnode, [], 0)])  # Each item in the queue is a tuple (node, path, cost)
  while queue:
    node, path, cost = queue.popleft()
    if node == endnode:
      print("Order of traversal from {} to {}: {}".format(startnode, endnode, " -> ".join(path + [node])))
      print("Total cost: {}".format(cost))
      return
    if node not in visited:
      visited.add(node)
      for adjacent_node, edge_cost in graph[node]:
        queue.append((adjacent_node, path + [node], cost + edge_cost))
  print("\nPath from {} to {} is not reachable.".format(startnode, endnode))
...
def main():
  ...
  while True:
    ...
    if option == 1:
      ...
      start_node = input("Enter the start node for the UCS: ")
      target_node = input("Enter the target node for the UCS: ")
